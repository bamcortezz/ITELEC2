	...
	class ADMIN
	{
		...
		public function verifyOTP($username, $email, $password, $tokencode, $otp, $csrf_token)
		{
			if ($otp == $_SESSION['OTP']) {
				...
				echo "<script> alert ('It appears that you entered is Invalid.'); window.location.href = '../../../verify-otp.php'; </script>";
				exit;
			}
		}
				...
				if ($stmt->rowCount() == 1) {
					if ($userRow['status'] == "active") {
						if ($userRow['password'] == md5($password)) {
							...
							echo "<script> alert ('Password incorrect.'); window.location.href = '../../../'; </script>";
							exit;
						}
					} else {
